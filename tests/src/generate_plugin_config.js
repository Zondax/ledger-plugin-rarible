const { getPluginConfigBasePath } = require("utils");
...
function generate_plugin_config(folderName, network="ethereum") {
	
	var fs = require('fs');
	var mainPath = getPluginConfigBasePath(folderName, network);
	var files = fs.readdirSync(`${mainPath}/abis/`);
	...
	for (let abiFileName of files) {
		...
		// Load abi
		let abi = require(`../${mainPath}/abis/${abiFileName}`);
		// Add it to contracts
		contracts_to_abis[contractAddress] = abi;
	}
	
	// Load the b2c.json file
	const b2c = require(`../${mainPath}/b2c.json`);
