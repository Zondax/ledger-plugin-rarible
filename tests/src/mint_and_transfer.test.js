...
// Nanos S test
test.each(assetNetworks)("%s - Mint and Transfer from Contract", async ({name, device, abi, contractAddr}) => {
    await zemu(device, name, async (sim, eth) => {
        ...
        const {data} = await contract.populateTransaction.mintAndTransfer(body, to);
        ...
        await sim.navigateAndCompareSnapshots('.', `${device}_${name}_mint_and_transfer_from_contract`, [device === "nanos" ? 11 : 7, 0]);
